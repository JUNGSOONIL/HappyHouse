    ...
    <div class="modal-dialog modal-dialog-centered modal-xl">
      <div class="modal-content">
        <div class="modal-header">
          <h5 class="modal-title"><strong>공지사항 수정</strong></h5>
          <button
            type="button"
            class="btn-close"
        ...
        <div class="modal-body">
          <div class="mb-3">
            <input
              v-model="$store.state.boardad.title"
              type="text"
              class="form-control"
              placeholder="제목"
            />
          ...
          <div v-if="$store.state.boardad.fileList.length > 0" class="mb-3">
            첨부파일 :
            <span
              ><div
        ...
        <div class="modal-footer">
          <button
            ...
            data-dismiss="modal"
            type="button"
            style="background-color: #656057; color: white"
          >
            수정
          </button>
        </div>
...
export default {
  ...
  methods: {
    ...
    boardUpdate() {
      ...
      formData.append("noticeId", this.$store.state.boardad.boardId); // update 에 추가
      formData.append("title", this.$store.state.boardad.title);
      ...
      http
        .post(
          "/notices/" + this.$store.state.boardad.boardId, // store boardId
          formData,
  ...
  mounted() {
    ...
    this.$el.addEventListener("show.bs.modal", function () {
      $this.CKEditor.setData($this.$store.state.boardad.content);
      // 첨부 파일 관련 초기화
