...
export default new Vuex.Store({
  ...
  mutations: {
    ...
    CLEAR_SIDO_LIST: (state) => {
      state.houses.sidos = [{ value: null, text: "선택하세요" }];
    },
    CLEAR_GUGUN_LIST: (state) => {
      state.houses.guguns = [{ value: null, text: "선택하세요" }];
    },
    CLEAR_DONG_LIST: (state) => {
      state.houses.dongs = [{ value: null, text: "선택하세요" }];
    },
    ...
    // 나랑 친구 관심 하우스 리스트 정보 저장
    SET_MY_INTEREST_LIST_MAIN(state, ints) {
      state.houses.maininterestlist = ints;
    },
    ...
    SET_POPULAR_HOUSE_LIST: (state, houses) => {
      state.houses.popularHouseList = houses;
    },
    ...
    SET_FRIEND_LIST(state, list) {
      // list.forEach((fId) => {
      //   state.friends.friendId.push({fId});
      // })
      state.friends.friendId = list;
    },
  },
  // for async method
  actions: {
    // 친구 리스트 가저오는 부분 ---------------------------------------------------
    friendList(context) {
      http.get("/friends/" + this.state.user.userId).then(({ data }) => {
        console.log("친구 리스트");
        console.log(data.friends);
        if (data.result == "login") {
          router.push("/login");
        } else {
          context.commit("SET_FRIEND_LIST", data.friends);
        }
      });
    },
    ...
    // 공지사항 글 가져오는 부분 --------------------------------------------------(여기 로그인 확인 해줘야하나?)
    boardListad(context) {
      http
        .get("/notices", {
          params: {
    ...
    getInterestListMain: ({ commit }, friendId) => {
      http
        .get("/interestsfriendlist/" + friendId)
        .then(({ data }) => {
          console.log("getInterestListMainData : ");
          console.log(data);
          if (data.result == "login") {
            commit("SET_USER", {
              isLogin: false,
            ...
            commit("SET_MY_INTEREST_LIST_MAIN", data);
          }
        })
        .catch((error) => {
          console.error("getInterestListError : ");
    ...
    getPopularList: ({ commit }) => {
      http
        .get("/interests")
        .then(({ data }) => {
          console.log("getPopularListData : ");
          console.log(data);
          commit("SET_POPULAR_HOUSE_LIST", data);
        })
        .catch((error) => {
          console.error("getPopularListError : ");
          console.error(error);
        });
    },
  },
  getters: {
    ...
    getPageCountad: function (state) {
      return Math.ceil(
        state.boardad.totalListItemCount / state.boardad.listRowCount
      );
    },
    getStartPageIndexad: function (state) {
      if (state.boardad.currentPageIndex % state.boardad.pageLinkCount == 0) {
        ...
        return (
          Math.floor(
            state.boardad.currentPageIndex / state.boardad.pageLinkCount
          ) *
            state.boardad.pageLinkCount +
          1
        );
