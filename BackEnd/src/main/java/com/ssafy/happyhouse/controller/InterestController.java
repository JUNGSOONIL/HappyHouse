...
public class InterestController {
	...
	public ResponseEntity<List<HouseInfoDto>> popList() throws Exception {
		if(service.getPopularList() != null) {
			return new ResponseEntity<List<HouseInfoDto>>(service.getPopularList(), HttpStatus.OK);
		} else {
			return new ResponseEntity<List<HouseInfoDto>>(service.getPopularList(), HttpStatus.INTERNAL_SERVER_ERROR);
		}
	}
	...
	@GetMapping(value="/interestsfriendlist/{userId}")
	public ResponseEntity<List<HouseInfoDto>>  interest(@PathVariable String userId) throws Exception{
		System.out.println("컨드롤");
		System.out.println(userId);
		List<HouseInfoDto> list = service.getInterestfriendList(userId);
